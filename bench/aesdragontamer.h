// Code adapted from
// https://github.com/lemire/testingRNG/blob/master/source/aesdragontamer.h
...
static inline void aesdragontamer_tobuffer_r(aesdragontamer_state *state) {
  ...
  _mm_storeu_si128((__m128i *)state->buffer,
                   penultimate1);  // stores 2 uint64_t
...
static inline void aesdragontamer_seed(aesdragontamer_state *state,
                                       uint64_t seed) {
  aesdragontamer_seed_r(state, seed);
}

static inline uint64_t aesdragontamer(aesdragontamer_state *state) {
  return aesdragontamer_r(state);
}
