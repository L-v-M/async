...
page_size_powers = [12, 14, 16, 18, 20, 22]
...
for page_size_power in page_size_powers:
    ...
    for threads, entries_per_ring in itertools.product(num_threads, num_entries_per_ring):
        subprocess.run([os.path.join(path_to_build_directory, "executables", "tpch_q1"),
                       lineitemq1, str(threads), str(entries_per_ring), str(1000), "true", "false", "false", print_header], check=True, stdout=query1_out, stderr=subprocess.PIPE)
        print_header = "false"

for page_size_power in [12, 16]:
    subprocess.run(["cmake", "-S", path_to_source_directory, "-B", path_to_build_directory, "-DCMAKE_BUILD_TYPE=Release",
                   "-DASYNCHRONOUS_IO_PAGE_SIZE_POWER={}".format(page_size_power), "-DCMAKE_CXX_COMPILER={}".format(path_to_cxx_compiler)], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    subprocess.run(
        ["cmake", "--build", path_to_build_directory, "--clean-first"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    for threads, entries_per_ring in itertools.product([10, 15, 20], num_entries_per_ring):
        subprocess.run([os.path.join(path_to_build_directory, "executables", "tpch_q1"),
                       lineitemq1, str(threads), str(entries_per_ring), str(1000), "true", "true", "false", "false"], check=True, stdout=query1_out, stderr=subprocess.PIPE)
